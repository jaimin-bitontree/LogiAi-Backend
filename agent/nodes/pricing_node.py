...
async def pricing_node(state: AgentState) -> AgentState:
    ...
    logger.info(f"[pricing_handler] Processing operator pricing email for {request_id}...")

    # Validate that we have a request_id from parse_node
    if not request_id:
        logger.error(f"[pricing_handler] No request_id in state.")
        return {"status" : "ERROR"}

    # 1. Extract pricing data from email body
    pricing_data, _ = extract_pricing_data(body)
    if not pricing_data:
        logger.error(f"[pricing_handler] Failed to extract pricing data from email.")
        return {"status" : "ERROR"}

    # 2. Get fresh shipment data from DB (to ensure we have latest)
    shipment = await find_by_request_id(request_id)
    if not shipment:
        logger.error(f"[pricing_handler] Shipment {request_id} not found in database.")
        return {"status" : "ERROR"}

    logger.info(f"[pricing_handler] Found shipment for customer: {shipment.customer_email}")

    customer_email = shipment.customer_email
    request_data = shipment.request_data or {}
    customer_name = request_data.get("required", {}).get("customer_name") or \
    ...
    try:
        ...
        logger.error(f"[pricing_handler] Failed to send email to customer: {e}")
        return {"status" : "ERROR"}
    except Exception as e:
        logger.error(f"[pricing_handler] Unexpected error while sending email: {e}")
        return {"status" : "ERROR"}
    ...
    return {
"status": "QUOTED",
"pricing_details": [pricing_data], # LangGraph appends this
"messages": [outgoing_msg], # add_messages reducer appends
"message_ids": [outgoing_message_id],
}
