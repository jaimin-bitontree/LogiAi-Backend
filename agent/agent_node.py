...
2. If result is_valid=false: Call send_missing_info_email with the missing_fields
3. If result is_valid=true: Call send_complete_info_emails
4. STOP - Do not call any more tools after sending email

For intent "missing_information":
1. Call extract_missing_field_values(request_id="REQ-...", missing_fields=[...])
2. If result is_valid=false: Call send_missing_info_email with still_missing fields
3. If result is_valid=true: Call send_complete_info_emails
4. STOP - Do not call any more tools after sending email
...
- STOP IMMEDIATELY after calling send_missing_info_email or send_complete_info_emails
- NEVER call extraction tools again after sending an email
- If extraction fails, still send the missing info email and STOP
"""
...
def call_agent(state: dict) -> dict:
            ...
            for attempt in range(2):
                try:
                    ...
                    if attempt == 0:
                        ...
                        print(f"\n🤖 [agent_node] Trying {model_name} with {key_label}")
                    else:
                        print(f"\n🤖 [agent_node] Retrying {model_name} (attempt {attempt + 1})")
                    ...
                    if "8b" in model_name.lower() and not response.tool_calls and response.content:
                        print(f"⚠️  [agent_node] 8B model returned text instead of tool call, skipping...")
                        logger.warning(f"[agent_node] 8B model failed to call tools")
                        break  # Don't retry, move to next model
                    
                    print(f"✅ [agent_node] Success with {model_name}")
                    print(f"🤖 [agent_node] Tool calls: {response.tool_calls}")
                    ...
                    if "rate_limit" in error_str.lower() or "429" in error_str:
                        ...
                        print(f"⚠️  [agent_node] Rate limit on API key {api_key_index + 1}")
                    ...
                    # Check if it's a tool calling error - retry once
                        ...
                        if attempt == 0:
                            ...
                            print(f"⚠️  [agent_node] Tool error, retrying...")
                            continue  # Retry same model
                        else:
                            logger.warning(f"[agent_node] Tool error persists, trying next model...")
                            print(f"⚠️  [agent_node] Tool error persists, trying next...")
    ...
    print(f"❌ [agent_node] All API keys exhausted. Last error: {last_error}")
    
    # Check if it was all rate limits
    if last_error and ("rate_limit" in str(last_error).lower() or "429" in str(last_error)):
        print(f"\n⚠️  All {len(GROQ_API_KEYS)} API key(s) are rate limited!")
        print("💡 Solutions:")
        print("   1. Wait 8-10 minutes for rate limits to reset")
        print("   2. Add more API keys to .env (GROQ_API_KEY_2, GROQ_API_KEY_3)")
        print("   3. Upgrade accounts to Groq Dev Tier")
        print("   4. Switch to OpenAI API\n")
