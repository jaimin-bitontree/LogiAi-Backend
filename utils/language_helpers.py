...
def get_detected_lang(shipment_doc) -> str:
    """Extract detected_language from a shipment document fetched from DB."""
    if not shipment_doc:
        return "en"
    
    # Handle both dict and Shipment model
    if isinstance(shipment_doc, dict):
        ...
        lang_meta = shipment_doc.language_metadata
        detected = lang_meta.detected_language if lang_meta else "en"
